									  ...
									  uint8_t *outPixels)
{
	...
	{
		outPixels[ix + iy * width] = 
			float(inPixels[(ix + iy * width) * 3 + 0]) * 0.299f +
			float(inPixels[(ix + iy * width) * 3 + 1]) * 0.587f +
			float(inPixels[(ix + iy * width) * 3 + 2]) * 0.114f;
	}
}
					 ...
					 bool useDevice = false, dim3 blockSize = dim3(1))
{
	...
	else // use device
	{
		...
		uint8_t *d_in, *d_out;
		CHECK(cudaMalloc(&d_in, width * height * 3));
		CHECK(cudaMalloc(&d_out, width * height));

		// TODO: Copy data to device memories
		CHECK(cudaMemcpy(d_in, inPixels, width * height * 3, cudaMemcpyHostToDevice));
		...
		CHECK(cudaMemcpy(outPixels, d_out, width * height, cudaMemcpyDeviceToHost));
