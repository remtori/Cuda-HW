...
__global__ void reduceBlksKernel1(int * in, int n, int * out)
{
    ...
    {
        if (threadIdx.x % stride == 0 && stride + i < n)
            in[i] += in[i + stride];
...
{	
    ...
    for (int stride = 1; stride < 2 * blockDim.x; stride *= 2)
    {
        ...
        if (threadIdx.x < blockDim.x / stride && i + (blockDim.x / stride) < n)
            in[i] += in[i + (blockDim.x / stride)];
        ...
        bool useDevice=false, dim3 blockSize=dim3(1), int kernelType=1)
{
	...
	else // Use device
	{
		...
		int * d_in, * d_out;
        dim3 gridSize((((n + 1)/ 2) + blockSize.x - 1) / blockSize.x);
