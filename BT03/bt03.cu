        ...
        uchar3 * outPixels)
{
	...
	for (int j = 0; j < filterWidth; j++) 
	{
		for (int i = 0; i < filterWidth; i++) 
		{
			...
			aX = min(max(0, aX), width - 1);
			aY = min(max(0, aY), height - 1);

			float filterVal = filter[i + j * filterWidth];
			uchar3 pixelVal = inPixels[aX + aY * width];
			sum.x += filterVal * pixelVal.x;
			sum.y += filterVal * pixelVal.y;
			sum.z += filterVal * pixelVal.z;
		}
	}

	outPixels[x + y * width] = make_uchar3(sum.x, sum.y, sum.z);
}
...
{	
	...
	int boundWidth = blockDim.x + filterWidth - 1;
	int boundHeight = blockDim.y + filterWidth - 1;

	for (int y = threadIdx.y; y < boundHeight; y += blockDim.y)
	{
		for (int x = threadIdx.x; x < boundWidth; x += blockDim.x)
		{
			int aX = min(max(x + offsetX - padding, 0), width - 1);
			int aY = min(max(y + offsetY - padding, 0), height - 1);
			s_inPixels[x + y * boundWidth] = inPixels[aX + aY * width];	
		}
	}
	...
	for (int j = 0; j < filterWidth; j++) 
	{
		for (int i = 0; i < filterWidth; i++) 
		{
			int aX = threadIdx.x + i;
			int aY = threadIdx.y + j;
			
			float filterVal = filter[i + j * filterWidth];
			uchar3 pixelVal = s_inPixels[aX + aY * boundWidth];
			sum.x += filterVal * pixelVal.x;
			sum.y += filterVal * pixelVal.y;
			sum.z += filterVal * pixelVal.z;
		}
        ...
        bool useDevice=false, dim3 blockSize=dim3(1, 1), int kernelType=1)
{
	...
	else // Use device
	{
		...
		{
			CHECK(cudaMemcpyToSymbol(dc_filter, filter, filterSize, 0, cudaMemcpyHostToDevice));
		}
		...
		else if (kernelType == 2)
		{
			blurImgKernel2
				<<<
					gridSize, 
					blockSize, 
					(blockSize.x + filterWidth - 1) * (blockSize.y + filterWidth - 1) * sizeof(uchar3)
				>>>
				(
